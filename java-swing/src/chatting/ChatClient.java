...
public class ChatClient extends JFrame implements ActionListener, Runnable, ItemListener{
	...
	OutputStream out;
	BufferedReader in;
	Socket socket; //서버는 소켓과 연관관계를 맺지 않지만 (서버는 소켓과 한몸이 아니지만_
	// 클라이언트는 반드시 소켓과 한몸이어야함 
	//(연관관계를 맺어야함, 소켓과 클라이언트는 부모-자식관계, 클라이언트가 죽으면 소켓도 죽음)
...
//		jp13.add(choiceIP);
		...
		this.setResizable(false);//창의 크기를 변경시키는 것이 불가능
		bar.add(menu);
		menu.add(itemOpen);
		menu.add(itemSave);
